...
distribution}. 
\nsm{Arghya: It might be helpful to delve into this a bit more, as some students might not be ready to recognize italicized font as being sort of  hint to the definition.
Maybe drawing a diagram showing sample space {H,T} on the left, possible values of pdf {1/2, 1/2} on the right and distribution function being an arrow/assignment between the two sides would bring more clarity.}
\nsm{I can't picture what you're describing; could you share a sketch?}
...
\begin{example}
    ...
    \new{Our first instinct might be to list out all of the possible 
    combinations of events; for each die roll, there is a tails and a 
    heads option: }
    ...
    \new{Each of these outcomes occurs with equal probability, and there 
    are twelve of them, so rolling a 1 and getting heads ($1H$) occurs 
    with probability $\frac{1}{12}$.}

    \new{If we realize that the dice and coin outcomes are independent, however,
...
be. 
\nsm{Arghya: This is just nitpicking, but should 
it be ``they want \emph{it} to be''? because the notion of security pertains to a scheme.}
\nsm{Well, the scheme is a general construction that is parametrized 
by $\lambda$, so a scheme has an (asymptotic) security that is different 
from the concrete security of an \emph{instantiation} with a particular 
$\lambda$. I agree that the sentence as-is is a bit awkwardly phrased, 
but I think it is more accurate this way since the security parameter 
affects how secure the particular instantiation of the scheme is, i.e. 
how secure the parties' communications with/application of (etc.) the 
scheme will be --- how secure they themselves will be. I don't want to 
use the word ``instantiation'', though, because I think the readers will
not be familiar with it. What do you think?}
...
\begin{example}\label{ex:floor}
    ...
    \new{When $s=37$, a similar thing happens: the scheme is correct for some 
    ...
    \begin{align*}
        ...
        &\vdots   &\vdots &
    \end{align*}
    So even though this scheme \emph{can} work for all $s \in \D$, \new{correctness 
    ...
    be a factor of $s$; $s_1$ can be 1 or $s$, so this holds even if $s$ is prime).}
\end{example}
...
\nsm{Arghya: I think it'd be helpful to describe a mock situation/thought exercise (parallel to the style of Bob and Alice kinda situations) before throwing in such detailed definition of the privacy game; maybe the explanation right after figure 2 can in incorporated in such description. I'm just slightly worried if we just give them this definition without enough motivation it might be somewhat dry and scary to the uninitiated reader of mathematical texts.}
\nsm{To be fair, this is an optional section so I only expected people 
who were happy to read a little bit more difficult text and concepts to 
venture here. But that's no reason to make things drier than they have to be :)
Here's an attempt.}
\new{More specifically, the secret sharing \emph{privacy game} captures 
...
\new{Let's think about the worst case, in which Eve is very powerful: she knows 
...
\new{What sort of protection does Alice need? Even with all her information 
(her knowledge of $x_0, x_1, i$, and $s_i$), we don't want Eve to figure
out whether she got a share of $x_0$ or $x_1$, because if she did, she'd
be able to unlock the safe!}

Here is the formal \emph{privacy game} for any secret sharing scheme $\ss =
...
\new{An important property of polynomials is that we can describe any one 
...
lines):\nsm{I changed the equation placement and added equations for the 
other polynomials. Does it look too crowded? Is the correspondence 
between equations and plots clear?}
...
\node [font=\small] at (8,3.3) {$f(x)=\frac{1}{4}x^2-x+3$};
\node [font=\small] at (-1,1.3) {$f(x)=-x^2+4x+3$};
\end{tikzpicture}
\end{center}
...
\nsm{@Arghya: you mentioned this should be $n$ instead of $n-1$. The top of 
the summation range is correctly labeled $n-1$, though, since we assume 
we are given $n$ points, not $n-1$, and the range starts at 0. I've updated 
the name of this variable to $m$ instead to avoid overloading $n$ and
to bring this notation more in line with Definition~\ref{def:ss-update}.}
