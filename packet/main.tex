...
speech. Something is random if its outcome is governed by what is called a 
...
\todo{(See Seyed's comments on Slack.)}
...
\begin{example}\label{ex:cond-vis}
    ...
    Throughout, let $B = \{3,4,7,8,11,12,15,16\}$ (the shaded cells).
...
\nsm{Seyed: Maybe a matter of taste. I would prefer not having them on top of each other and just having them next to each other. Also, maybe dropping the figure is not a bad idea.}
\todo{(Check with Mamma/Trevor.)}
...
\new{What sort of protection does Alice need? Even with all the information 
