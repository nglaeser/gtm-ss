...
variable}, since it can take on one of several values depending based on an underlying 
...
To find the probability of one event or another happening, we add probabilities.
For example, the probability that $c$ is heads or tails is

\[
    \Pr[c=H] + \Pr[c=T] = \frac{1}{2} + \frac{1}{2} = 1
\]
...
When the value of one random variable is not related to that of another, we say 
the variables are \emph{independent}. For example, a good coin toss should be 
independent of the coin used for the toss or the person doing the toss.
When two random variables are independent, we can multiply the probabilities 
...
Remember, for \emph{or}, we add; for \emph{and}, we multiply.

% \begin{center}
% \fbox{\parbox{8cm}{\centering 
% \textbf{or} means $+$\\
% \textbf{and} means $\cdot$
% }}
% \end{center}
...
given that another event occurred. This is written with the symbol 
$\mid$, which is read aloud as ``given''. So, $\Pr[A \mid B]$ is 
read as ``the probability that $A$ given $B$''.
...
When two events A and B are independent, the following equality 
holds:
\begin{equation}
...
\begin{example}
    Suppose I only flip the coin again if my first coin toss came out heads. 
    ...
    (H), and we don't do another coin toss, or (2) the first toss comes uptails,
    and the second one comes up heads (TH). 
    
    Now we need to use conditional probability, since our second coin toss
    depends on the first:
    \begin{align*}
        & \Pr[H] + \Pr[TH]\\
        =& \Pr[H] + \Pr[TH \mid \text{first toss is }T]\cdot \Pr[\text{first toss is }T]
    \end{align*}
...
\begin{exercise}
    ...
    for each pair of probability distributions:
    \renewcommand{\labelenumi}{(\alph{enumi})} 
    \begin{enumerate}
        \item Rolling and even or an odd number on a 6-sided die, 
        getting heads or tails when you flip a coin
        \item Drawing a random card from a deck, rolling a 6-sided 
        die and flipping 3 coins
        \item Whether or not you get at least one heads when flipping
        two coins, whether or not you draw a club from a shuffled 
        deck.
    \end{enumerate}
\end{exercise}
...
% TODO
% This abstract notion of secret sharing is a \emph{cryptographic 
% primitive}. Other cryptographic primitives include public-key encryption
% and symmetric-key encryption. A primitive consists of an outline and 
% set of properties and could be \emph{instantiated} by many different 
% schemes. For example, the additive secret sharing scheme in Figure~\ref{fig:additiveSS}
% is only one way of sharing secrets. Secret sharing as a primitive was 

Here's a visual representation: \nsm{Might labeling the right side $\D_s^n$ 
be confusing?}
...
\begin{example}\label{ex:floor3}
    ...
    rounds a decimal down to the integer below it. For example, $\lfloor 3.725 
    \rfloor = 3$.
...
\begin{example}
    So setting $s_1$ to always be 3 works when sharing some numbers but not 
    ...
    % }
    \end{pchstack}
    This is still meets Definition~\ref{def:ss}. Is the adjusted scheme correct
    now?
    ...
    could fail randomly depending on the choice of $s_1$. So it is still 
    not correct.
\end{example}
...
\begin{example}
    We already saw that the scheme in Example~\ref{ex:floor3} is not correct.
    But is it private?

    You might think so at first, since the number 3 (the first share) is
    unrelatedd to the secret, and the second share isn't giving away the 
    secret if the adversary doesn't know that it's just the secret minus 3.
    But in reality, we can't assume that the adversary doesn't know the 
    inner workings of the \share~algorithm. 
    ...
    wins with probability 1, so its advantage is $\frac{1}{2}$, so the 
...
the symbols, $\sum_{i=0}^{n-1}$ means to evaluate the expression after 
$\sum$ for each value of $i$ and then add all those terms together. For 
example:
...
$\prod$ is similar, but we multiply the expressions instead of adding 
them:
\[
...
\begin{bonus}
    ...
    \begin{enumerate}
        ...
        elements of the set $\{1, 0, 5, -3, 8\}$ and the numbering
...
\begin{example}
Let's do the degree-3 example from Section~\ref{sec:unique} next. Our set of 
...
% TODO explain - Why is the actual threshold $t+1$ and not simply $t$? 
