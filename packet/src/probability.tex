...
A variable that represents an outcome or event (like the outcome $c$ of the coin toss we talked about earlier) is called a \emph{random 
...
$\frac{1}{2}$. (Another uniform distribution, this time for the sample space $\{1, 2, 3, 4, 5, 6\}$, is shown in Example~\exref{ex:prob-dist}b above). We say such a variable is ``sampled uniformly at random'' 
...
A quick aside about notation: going forward, when it is clear from context what random variable we are referring to, we'll leave the random variable out of our probability expressions. For example, instead of writing $\Pr[c=H]$ we'll just write $\Pr[H]$.

\setlength\fboxsep{1em}        
\begin{center}
\fbox{
    \begin{minipage}{.7\linewidth}
        \textsc{``Or'' rule of probabilities.} \\
...
\[
    \Pr[\text{raining} \mid \text{cloudy sky}] \geq
...
\begin{example}\label{ex:cond-vis}
    ...
    \begin{enumerate}
        \item Suppose $A = \{9,10\}$:
            ...
            we can justify this by saying that $A$ occupies none (``0'') of the space
            of $B$.
...
\fbox{
    \begin{minipage}{.7\linewidth}
        \textsc{``And'' rule of probabilities.} \\
...
distribution and sample space, we say they are \new{\emph{perfectly indistinguishable}. 
...
some \emph{cryptographic primitive} (a general concept with desirable properties, like a sort of ``blueprint''). For example, the Caesar 
...
to compute something, it is instead said to be \emph{computationally secure}.}